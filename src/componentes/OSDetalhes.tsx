    ...
    <Dialog open={aberto} onOpenChange={onFechar}>
      <DialogContent className="bg-slate-800 border-slate-700 max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-6">
          ...
          {(os.materialAguardando || os.dataEntregaMaterial) && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="bg-slate-700/30 rounded-lg p-4">
                <div className="flex items-center gap-2 text-slate-400 mb-1">
                  <Wrench className="w-4 h-4" />
                  <span className="text-sm">Peça/Material Aguardado</span>
                </div>
                <p className="text-white font-medium">{os.materialAguardando || '-'}</p>
              </div>
              <div className="bg-slate-700/30 rounded-lg p-4">
                <div className="flex items-center gap-2 text-slate-400 mb-1">
                  <Calendar className="w-4 h-4" />
                  <span className="text-sm">Prazo de Chegada</span>
                </div>
                <p className="text-white font-medium">
                  {os.dataEntregaMaterial ? formatarDataSomente(os.dataEntregaMaterial) : 'Não definido'}
                </p>
              </div>
            </div>
          )}
